...
// Benchmark and test parameters  
    ...
    #define SMALL_BENCH_LOOPS       5       // Number of iterations per bench
    #define TEST_LOOPS             10       // Number of iterations per test
    #define ECPT_TEST_LOOPS        10       // Number of iterations per EC point generation test
    #define ECPAIR_TEST_LOOPS      10       // Number of iterations per pairing test
...
bool ecisog_run(PCurveIsogenyStaticData CurveIsogenyData)
{
    ...
    printf("  Point doubling runs in .......................................... %7lld ", msrcycles/BENCH_LOOPS); print_unit;
    printf("\n\n");
    ...
    printf("  4-isogeny of projective point runs in ........................... %7lld ", msrcycles/BENCH_LOOPS); print_unit;
    printf("\n\n");
    ...
    printf("  3-isogeny of projective point runs in ........................... %7lld ", msrcycles/BENCH_LOOPS); print_unit;
    printf("\n\n");
    ...
    printf("  3-isogeny evaluation at projective point runs in ................ %7lld ", msrcycles/BENCH_LOOPS); print_unit;
    printf("\n");
    ...
    for (n = 0; n < SMALL_BENCH_LOOPS; n++)
    {
        ...
        cycles1 = cpucycles(); 
        generate_2_torsion_basis(A, R1, R2, CurveIsogeny); // Original
        cycles2 = cpucycles();
        msrcycles = msrcycles+(cycles2-cycles1);
    ...
    for (n = 0; n < SMALL_BENCH_LOOPS; n++)
    {
        ...
        cycles1 = cpucycles(); 
        generate_3_torsion_basis(A, R1, R2, CurveIsogeny); // Original
        cycles2 = cpucycles();
        msrcycles = msrcycles+(cycles2-cycles1);
...
bool discretelog_run(PCurveIsogenyStaticData CurveIsogenyData) {
    long long cycles, cycles1, cycles2, msrcycles;
    digit_t digit[NWORDS_ORDER];
    f2elm_t t_ori[5], epq, m[5], n[4], LUT[4], LUT_0[4], LUT_1[5], LUT_B[5], LUT_1_B[4], LUT_3_B[6];
    f2elm_t A, one = {0};
    f2elm_t vec[4], Zinv[4];
    point_t phiP, phiQ, S1, S2;
    ...
    for (int i = 0; i < SMALL_BENCH_LOOPS; i++) {
        ...
        to_fp2mont(((f2elm_t*)PublicKeyB)[1], ((f2elm_t*)&PK_B)[1]); 
        to_fp2mont(((f2elm_t*)PublicKeyB)[2], ((f2elm_t*)&PK_B)[2]); 

        recover_y(PK_B, phP, phQ, phX, A, CurveIsogeny);
        ...
        fp2mul751_mont(Q->X, Zinv[1], S2->x);
        fp2mul751_mont(Q->Y, Zinv[1], S2->y);        
    ...
    for (int i = 0; i < SMALL_BENCH_LOOPS; i++) {
        ...
        to_fp2mont(((f2elm_t*)PublicKeyA)[1], ((f2elm_t*)&PK_A)[1]); 
        to_fp2mont(((f2elm_t*)PublicKeyA)[2], ((f2elm_t*)&PK_A)[2]); 
    ...
    for (int i = 0; i < SMALL_BENCH_LOOPS; i++) {
        ...
        fp2mul751_mont(phP->X, Zinv[0], phiP->x);
        fp2mul751_mont(phP->Y, Zinv[0], phiP->y);
        ...
        cycles1 = cpucycles(); 
        to_fp2mont(m[0],epq);
        build_LUTs(epq, t_ori, LUT_B, LUT_0, LUT_1_B, LUT_3_B, one[0]);
        phn84(m[1], t_ori, LUT_B, LUT_0, LUT_1_B, LUT_3_B, one[0], digit);
        cycles2 = cpucycles();
        msrcycles = msrcycles + (cycles2-cycles1);
